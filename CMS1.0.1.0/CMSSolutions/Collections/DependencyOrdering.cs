...
namespace CMSSolutions.Collections
{
    public static class DependencyOrdering
    {
        ...
        /// </summary>
        public static IEnumerable<T> OrderByDependenciesAndPriorities<T>(
            this IEnumerable<T> elements,
            Func<T, T, bool> hasDependency,
            Func<T, int> getPriority)
        {
            Linkage<T>[] population = elements.Select(d => new Linkage<T>
            {
                Element = d
            }).OrderBy(item => getPriority(item.Element)).ToArray();

            // Performing an initial sorting by priorities may optimize performance
        ...
        private static void Add<T>(
            Linkage<T> item,
            ICollection<T> list,
            IEnumerable<Linkage<T>> population,
            Func<T, T, bool> hasDependency,
            Func<T, int> getPriority)
        {
            if (item.Used)
        ...
        private static int BestPriorityPosition<T>(
            List<T> list,
            int index,
            Func<T, T, bool> hasDependency,
            Func<T, int> getPriority)
