...
namespace CMSSolutions.ContentManagement.Aliases.Implementation.Map
{
    [Feature(Constants.Areas.Aliases)]
    public class AliasMap
    {
        private readonly string area;
        private readonly ConcurrentDictionary<string, IDictionary<string, string>> aliases;
        private readonly Node root;

        public AliasMap(string area)
        {
            this.area = area;
            aliases = new ConcurrentDictionary<string, IDictionary<string, string>>(StringComparer.OrdinalIgnoreCase);
            root = new Node();
        }

        public IEnumerable<AliasInfo> GetAliases()
        {
            return aliases.Select(x => new AliasInfo { Area = area, Path = x.Key, RouteValues = x.Value });
        }

        public bool TryGetAlias(string virtualPath, out IDictionary<string, string> routeValues)
        {
            return aliases.TryGetValue(virtualPath, out routeValues);
        }

        public Tuple<IDictionary<string, object>, string> Locate(RouteValueDictionary routeValues)
        {
            return Traverse(root, routeValues, area);
        }
        ...
        public void Insert(AliasInfo info)
        {
            ...
            aliases[info.Path] = info.RouteValues;
            ExpandTree(root, info.Path, info.RouteValues);
        }
        ...
        public void Remove(AliasInfo info)
        {
            ...
            aliases.TryRemove(info.Path, out values);
            CollapseTree(root, info.Path, info.RouteValues);
        }
        ...
        private class Node
        {
            ...
            public ConcurrentDictionary<string, ConcurrentDictionary<string, Node>> Stems { get; private set; }

            public ConcurrentDictionary<string, object> Paths { get; private set; }
        }
    }
}