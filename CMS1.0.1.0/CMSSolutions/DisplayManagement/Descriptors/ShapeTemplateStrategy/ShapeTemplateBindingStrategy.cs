...
namespace CMSSolutions.DisplayManagement.Descriptors.ShapeTemplateStrategy
{
    public class ShapeTemplateBindingStrategy : IShapeTableProvider
    {
        ...
        {
            Logger.Info("Start discovering shapes");
            ...
            {
                Logger.Info("Start discovering candidate views filenames");
                var pathContexts = harvesterInfos.SelectMany(harvesterInfo => harvesterInfo.subPaths.Select(subPath =>
                {
                    var basePath = extensionDescriptor.Location.Replace(Path.DirectorySeparatorChar, '/');
                ...
                Logger.Info("Done discovering candidate views filenames");
            ...
            foreach (var iter in hits)
            {
                ...
                {
                    Logger.DebugFormat("Binding {0} as shape [{1}] for feature {2}",
            ...
            Logger.Info("Done discovering shapes");
        }

        private bool FeatureIsEnabled(FeatureDescriptor fd)
        {
            return (DefaultExtensionTypes.IsTheme(fd.Extension.ExtensionType) && (fd.Id == "Dashboard" || fd.Id == "Default")) ||
        ...
        {
            Logger.InfoFormat("Rendering template file '{0}'", harvestShapeInfo.TemplateVirtualPath);
            ...
            Logger.InfoFormat("Done rendering template file '{0}'", harvestShapeInfo.TemplateVirtualPath);
            return result;
        }
    }
}