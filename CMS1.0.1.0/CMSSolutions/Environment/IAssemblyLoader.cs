...
namespace CMSSolutions.Environment
{
    ...
    public class DefaultAssemblyLoader : IAssemblyLoader
    {
        ...
        private readonly ConcurrentDictionary<string, Assembly> loadedAssemblies = new ConcurrentDictionary<string, Assembly>(StringComparer.OrdinalIgnoreCase);

        public DefaultAssemblyLoader(IEnumerable<IAssemblyNameResolver> assemblyNameResolvers)
        {
            this.assemblyNameResolvers = assemblyNameResolvers.OrderBy(l => l.Order);
        ...
        public Assembly Load(string assemblyName)
        {
            try
            {
                return loadedAssemblies.GetOrAdd(this.ExtractAssemblyShortName(assemblyName), shortName => LoadWorker(shortName, assemblyName));
            }
            catch (Exception e)
            {
                Logger.ErrorFormat(e, "Error loading assembly '{0}'", assemblyName);
                return null;
            }
        }
        ...
        private Assembly LoadWorker(string shortName, string fullName)
        {
            ...
            var resolvedName = assemblyNameResolvers.Select(r => r.Resolve(shortName)).FirstOrDefault(f => f != null);
