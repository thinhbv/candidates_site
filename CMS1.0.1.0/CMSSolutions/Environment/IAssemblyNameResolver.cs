...
namespace CMSSolutions.Environment
{
    ...
    public class FrameworkAssemblyNameResolver : IAssemblyNameResolver
    {
        ...
        public string Resolve(string shortName)
        {
            // A few common .net framework assemblies are referenced by the CMSSolutions assembly.
            // Look into those to see if we can find the assembly we are looking for.
            var frameworkReferences = cacheManager.Get(typeof(IAssemblyLoader), ctx =>
                            ctx.Key.Assembly
            ...
            if (frameworkReferences.TryGetValue(shortName, out assemblyName))
            {
    ...
    public class GacAssemblyNameResolver : IAssemblyNameResolver
    {
        ...
        public string Resolve(string shortName)
        {
            ...
            var lookup = cacheManager.Get("---", ctx =>
