...
namespace CMSSolutions.Events
{
    public class EventsInterceptor : IInterceptor
    {
        ...
        public void Intercept(IInvocation invocation)
        {
            ...
            var results = eventBus.Notify(interfaceName + "." + methodName, data);
        ...
        public static object Adjust(IEnumerable results, Type returnType)
        {
            ...
            var enumerableOfTypeT = enumerableOfTypeTDictionary.GetOrAdd(returnType, type => typeof(Enumerable).GetGenericMethod("OfType", type.GetGenericArguments(), new[] { typeof(IEnumerable) }, typeof(IEnumerable<>)));
            return enumerableOfTypeT.Invoke(null, new object[] { results });
        }
    }
