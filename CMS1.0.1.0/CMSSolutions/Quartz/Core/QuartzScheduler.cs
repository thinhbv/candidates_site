...
using CMSSolutions.Quartz.Impl.Triggers;
using CMSSolutions.Quartz.Listener;
using CMSSolutions.Quartz.Simpl;
using CMSSolutions.Quartz.Spi;

namespace CMSSolutions.Quartz.Core
{
    /// <summary>
    ...
    public class QuartzScheduler : MarshalByRefObject, IRemotableQuartzScheduler
    {
        ...
        private class DelayedSchedulerStarter
        {
            ...
            public DelayedSchedulerStarter(QuartzScheduler scheduler, TimeSpan delay, Logger logger)
            {
                ...
                foreach (IJobExecutionContext job in jobs)
                {
                    if (job.JobInstance is IInterruptableJob)
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            log.Warn("Encountered error when interrupting job {0} during shutdown: {1}", job.JobDetail.Key, ex);
                        }
                    }
                }
            }
