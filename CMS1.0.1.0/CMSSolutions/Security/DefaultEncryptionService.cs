...
namespace CMSSolutions.Security
{
    public class DefaultEncryptionService : IEncryptionService
    {
        ...
        {
            var algorithm = SymmetricAlgorithm.Create(shellSettings.EncryptionAlgorithm);
            algorithm.Key = ToByteArray(shellSettings.EncryptionKey);
            return algorithm;
        }

        private HMAC CreateHashAlgorithm()
        {
            var algorithm = HMAC.Create(shellSettings.HashAlgorithm);
            algorithm.Key = ToByteArray(shellSettings.HashKey);
            return algorithm;
        }
