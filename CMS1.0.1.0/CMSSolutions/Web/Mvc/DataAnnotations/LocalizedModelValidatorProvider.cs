...
namespace CMSSolutions.Web.Mvc.DataAnnotations
{
    public class LocalizedModelValidatorProvider : DataAnnotationsModelValidatorProvider
    {
        private static readonly Dictionary<Type, Func<ValidationAttribute, Localizer, ValidationAttribute>> validationAttributes;

        static LocalizedModelValidatorProvider()
        {
            validationAttributes = new Dictionary<Type, Func<ValidationAttribute, Localizer, ValidationAttribute>> {
        ...
        protected override IEnumerable<ModelValidator> GetValidators(ModelMetadata metadata, ControllerContext context, IEnumerable<Attribute> attributes)
        {
            ...
            foreach (var attribute in attributes)
            {
                ...
                // default translations use the attribute's scope, e.g., LocalizedRequiredAttribute
                ...
                if (validationAttributes.TryGetValue(attribute.GetType(), out localizedAttribute))
                {
