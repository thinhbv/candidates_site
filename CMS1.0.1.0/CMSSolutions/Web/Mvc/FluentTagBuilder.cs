...
namespace CMSSolutions.Web.Mvc
{
    //Example useage:
    /*
     * var html = new FluentTagBuilder("html")
                .StartTag("head")
                    .StartTag("title")
                    .MergeAttributes(new Dictionary<string, object>()   //add attributes with dictionary
                    {
                        { "class", "something" }
                    })
                    .MergeAttributes(new   //add attributes with new anonymous object
                    {
                        data_val = "baloo"
                    })
                    .SetInnerText("Test 123")
                    .EndTag()
                .EndTag()
                .StartTag("body")
                    .StartTag("p")
                    .SetInnerText("hello fluent tag builder!")
                    .EndTag()
                .EndTag()
                .ToString();
    */
