...
namespace CMSSolutions.Web.Mvc.ViewEngines.Razor
{
    ...
    public class DefaultRazorCompilationEvents : IRazorCompilationEvents
    {
        ...
        public void CodeGenerationStarted(RazorBuildProvider provider)
        {
            ...
            IEnumerable<DependencyDescriptor> dependencyDescriptors = dependenciesFolder.LoadDescriptors();
            ...
            var entries = filteredDependencyDescriptors
                .SelectMany(descriptor => loaders
                                              ...
                                              .Select(loader => new
                                              {
                                                  ...
                                                  dependencies = extensionDependenciesManager.GetVirtualPathDependencies(descriptor.Name)
                                              }));
            ...
            foreach (var entry in entries)
            {
                foreach (var reference in entry.references)
                {
                    ...
                    {
                        var assembly = assemblyLoader.Load(reference.AssemblyName);
                        if (assembly != null)
                    ...
                    if (!string.IsNullOrEmpty(reference.BuildProviderTarget))
                    {
                        ...
                        var assembly = buildManager.GetCompiledAssembly(reference.BuildProviderTarget);
                        if (assembly != null)
                            assembliesToAdd.Add(assembly);
                    }
                }
            ...
            if (Logger.IsDebugEnabled)
            {
                ...
                {
                    Logger.DebugFormat("CodeGenerationStarted(\"{0}\") - no dependencies.", provider.VirtualPath);
                }
                else
                {
                    Logger.DebugFormat("CodeGenerationStarted(\"{0}\") - Dependencies: ", provider.VirtualPath);
                    
                    foreach (var virtualPath in provider.VirtualPathDependencies)
                    {
                        Logger.DebugFormat("  VirtualPath: \"{0}\"", virtualPath);
                    }

                    foreach (var assembly in assembliesToAdd)
                    {
                        Logger.DebugFormat("  Reference: \"{0}\"", assembly);
                    }
                }
            }
        }
        ...
        private DependencyDescriptor GetModuleDependencyDescriptor(string virtualPath)
        {
            ...
            return dependenciesFolder.GetDescriptor(moduleName);
        }
