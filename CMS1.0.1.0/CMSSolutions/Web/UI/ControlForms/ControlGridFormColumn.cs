using System;

namespace CMSSolutions.Web.UI.ControlForms
{
    public abstract class ControlGridFormColumn
    {
        internal string HeaderText { get; set; }

        public string PropertyName { get; set; }
        
        public Type PropertyType { get; set; }

        internal int? Width { get; set; }

        public string Align { get; set; }
        
        public string CssClass { get; set; }

        public abstract string BuildHtml(object item);
    }

    public class ControlGridFormColumn<TModel> : ControlGridFormColumn
    {
        internal string DisplayFormat { get; set; }

        internal bool Sortable { get; set; }

        internal bool Filterable { get; set; }

        internal Func<TModel, string> HtmlBuilder { get; private set; }

        internal Func<TModel, dynamic> ValueGetter { get; private set; }

        internal ControlGridFormColumn()
        {
            Sortable = true;
            Align = "left";
        }

        internal void SetValueGetter<TValue>(Func<TModel, TValue> expression)
        {
            ValueGetter = model => expression(model);

            HtmlBuilder = model =>
            {
                dynamic value;

                try
                {
                    value = ValueGetter(model);
                }
                catch (NullReferenceException)
                {
                    value = null;
                }

                if (value == null)
                {
                    return string.Empty;
                }

                return !string.IsNullOrEmpty(DisplayFormat) ? string.Format(DisplayFormat, value) : Convert.ToString(value);
            };
        }

        public ControlGridFormColumn<TModel> HasText(Func<TModel, string> expression)
        {
            SetValueGetter(expression);
            return this;
        }

        public ControlGridFormColumn<TModel> HasDisplayFormat(string value)
        {
            DisplayFormat = value;
            return this;
        }

        public ControlGridFormColumn<TModel> HasHeaderText(string value)
        {
            HeaderText = value;
            return this;
        }

        public ControlGridFormColumn<TModel> HasCssClass(string value)
        {
            CssClass = value;
            return this;
        }

        public ControlGridFormColumn<TModel> EnableFilter(bool value = true)
        {
            if (value)
            {
                if (!PropertyType.IsPrimitive && PropertyType != typeof(string))
                {
                    throw new NotSupportedException("Cannot enable filter for: " + PropertyName);
                }
            }
            Filterable = value;
            return this;
        }

        public ControlGridFormColumn<TModel> HasWidth(int value)
        {
            Width = value;
            return this;
        }

        public ControlGridFormColumn<TModel> RenderAsHtml(Func<TModel, string> builder)
        {
            HtmlBuilder = builder;
            return this;
        }

        public ControlGridFormColumn<TModel> RenderAsLink(Func<TModel, string> href, bool targetBlank = true)
        {
            if (targetBlank)
            {
                HtmlBuilder = model => string.Format("<a href=\"{0}\" target=\"_blank\">{0}</a>", href(model));
            }
            else
            {
                HtmlBuilder = model => string.Format("<a href=\"{0}\">{0}</a>", href(model));
            }
            return this;
        }

        public ControlGridFormColumn<TModel> RenderAsLink(Func<TModel, string> text, Func<TModel, string> href, bool targetBlank = true)
        {
            if (targetBlank)
            {
                HtmlBuilder = model => string.Format("<a href=\"{0}\" target=\"_blank\">{1}</a>", href(model), text(model));
            }
            else
            {
                HtmlBuilder = model => string.Format("<a href=\"{0}\">{1}</a>", href(model), text(model));
            }
            return this;
        }

        public ControlGridFormColumn<TModel> RenderAsImage(Func<TModel, string> src)
        {
            HtmlBuilder = model => string.Format("<img src=\"{0}\" />", src(model));
            return this;
        }

        public ControlGridFormColumn<TModel> RenderAsImage(Func<TModel, string> src, string cssClass)
        {
            HtmlBuilder = model => string.Format("<img class=\"{0}\" src=\"{1}\" />", cssClass, src(model));
            return this;
        }

        public ControlGridFormColumn<TModel> RenderAsStatusImage(string trueImageSrc, string falseImageSrc)
        {
            HtmlBuilder = model =>
            {
                var value = Convert.ToBoolean(ValueGetter(model));
                return string.Format("<img src=\"{0}\" />", value ? trueImageSrc : falseImageSrc);
            };
            return this;
        }

        public ControlGridFormColumn<TModel> RenderAsStatusImage(bool reverse = false, bool showTrueOnly = false)
        {
            const string success =
                "data:image/gif;base64,";
            const string block =
                "data:image/png;base64,";

            HtmlBuilder = model =>
            {
                var value = Convert.ToBoolean(ValueGetter(model));
                if (showTrueOnly)
                {
                    var trueImageSrc = reverse ? block : success;
                    return value ? string.Format("<img src=\"{0}\" />", trueImageSrc) : null;
                }
                else
                {
                    var trueImageSrc = reverse ? block : success;
                    var falseImageSrc = reverse ? success : block;
                    return string.Format("<img src=\"{0}\" />", value ? trueImageSrc : falseImageSrc);
                }
            };
            return this;
        }

        public ControlGridFormColumn<TModel> RenderAsStatusImage(Func<TModel, bool> valueGetter, bool reverse = false, bool showTrueOnly = false)
        {
            const string success =
                "data:image/gif;base64,";
            const string block =
                "data:image/png;base64,";

            HtmlBuilder = model =>
            {
                var value = valueGetter(model);
                if (showTrueOnly)
                {
                    var trueImageSrc = reverse ? block : success;
                    return value ? string.Format("<img src=\"{0}\" />", trueImageSrc) : null;
                }
                else
                {
                    var trueImageSrc = reverse ? block : success;
                    var falseImageSrc = reverse ? success : block;
                    return string.Format("<img src=\"{0}\" />", value ? trueImageSrc : falseImageSrc);
                }
            };
            return this;
        }

        public ControlGridFormColumn<TModel> EnableSorting(bool value = true)
        {
            Sortable = value;
            return this;
        }

        public ControlGridFormColumn<TModel> AlignLeft()
        {
            Align = "left";
            return this;
        }

        public ControlGridFormColumn<TModel> AlignCenter()
        {
            Align = "center";
            return this;
        }

        public ControlGridFormColumn<TModel> AlignRight()
        {
            Align = "right";
            return this;
        }

        public override string BuildHtml(object item)
        {
            return HtmlBuilder((TModel)item);
        }
    }
}